...
When encountering a repo for the first time:

1. **Ask** — does the user want a full overview or a partial/targeted overview?
2. **Document** — if overview is requested, create a `.claude/docs/` directory with well-organized documentation about the project.
3. **Diagram** — include Mermaid diagrams where useful (architecture, data flow, component relationships, sequence diagrams).
4. **Keep it current** — documentation is living; update it as the project changes.
